...
BOARD_TYPE=qca
CHIP_TYPE=isisc
# SET OF LAN PHY PORT: LAN1->2 LAN2->3 LAN3->4 LAN4->5
LAN_PHY_PORT_SET="2 3 4 5"
# SET OF WAN PHY PORT: WAN->1
WAN_PHY_PORT_SET="1"
...
create_hybrid_vlan()
{
	...
	for lan in $LAN_PHY_PORT_SET;do
		[ -z "$lan" ] && continue
		create_vdevice "eth0" $lan $lan "*" "0" "t"
	done

	for wan in $WAN_PHY_PORT_SET;do
		[ -z "$wan" ] && continue
		create_vdevice "eth0" 4094 $wan "*" "0" "t"
	done
	flush_fdb
}
