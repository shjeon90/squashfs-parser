...
config_country() {
	...
	local country_getfirm=$(getfirm COUNTRY)
	...
	if [ `expr "$country" : '[0-9].*'` -ne 0 ]; then
		if [ "$country" = "840" -a "$country_getfirm" = "TW" ];then
			country="158"
		fi
		iwpriv "$1" setCountryID "$country"
	elif [ -n "$country" ]; then
		if [ "$country" = "US" -a "$country_getfirm" = "TW" ];then
			country="TW"
		fi	
	...
	for dev in $DEVICES; do
		...
		for vif in $(config_get "$dev" vifs); do 
			if [ -d /sys/class/net/$vif ]; then
				...
				local ffs_wireless_iface=$(uci get amazon_ffs.ffs.wireless_iface -q)
				if [ "$ffs_support" != yes -o "$vif" != "$ffs_wireless_iface" ]; then
...
create_vap() {
	...
	if [ "$vif" = "ath03" -o "$vif" = "ath13" ]; then
		[ -d /sys/class/net/$vif ] || [ "$vif" = "$(/usr/sbin/wlanconfig "$vif" create wlandev "$dev" wlanmode "$mode" nosbeacon vapid 2)" ] || return 0
	else
	...
	[ "$onemesh_enable" = "on" ] && [ "$onemesh_ie" = "on" ] && [ "$sysmode" = "router" ] && {
		iwpriv "$vif" setiebuf 0x0 0x0 0x0 0x0 0x0 0x0 0x0 0x20 0xdd 0x1e 0x0 0x1d 0x0f 0x10 0x1 0x7 0x0 0x0 $tpie_mac $tpie_mac $random_suffix 0x0 0x0 ${tpie_mac:21:9} 0x0 0x1 0x0 0x0
		iwpriv "$vif" setiebuf 0x0 0x0 0x0 0x2 0x0 0x0 0x0 0x20 0xdd 0x1e 0x0 0x1d 0x0f 0x10 0x1 0x7 0x0 0x0 $tpie_mac $tpie_mac $random_suffix 0x0 0x0 ${tpie_mac:21:9} 0x0 0x1 0x0 0x0
	}
...
config_vap() {
	...
	#make authmode clear after wep key clear to fix dhcp failure when switch to no security from wep.
...
wifi_disconnect_stas()
{
	for dev in $DEVICES; do
		config_get vifs "$dev" vifs
		for vif in $vifs; do 
			#extern for single vif by first parameter
			if [ -z "$1" -o "$vif" == "$1" ]; then
...
wifi_reload() {
	...
	wifi_radio "$1" "txchainmask rxchainmask AMPDU ampdudensity !AMSDU AMPDULim AMPDUFrames AMPDURxBsize !bcnburst:set_bcnburst=0 \
		...
		!led_enable promisc block_interbss aggr_burst set_pmf txbf_snd_int=100 mcast_echo obss_rssi_th obss_rx_rssi_th disablestats=0 !enable_macreq=1" \
		&>$STDOUT
